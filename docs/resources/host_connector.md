...
- `profile` (String, Sensitive, Deprecated) OpenVPN profile of the connector. Deprecated: use the `cloudconnexa_host_connector_profile` ephemeral resource, which retrieves the profile without storing it in state.
- `token` (String, Sensitive, Deprecated) Connector token, set when the connector is created and not refreshed afterwards; empty for imported connectors. Deprecated: use the `cloudconnexa_host_connector_token` ephemeral resource, which mints a token without storing it in state.
